...
PATH_TO_CKPT = \
    '/media/spectra/GMS/CarColorDataset/downloads/frozen_inference_graph.pb'
...
with detection_graph.as_default():
    ...
    for file in glob.glob("{}/*".format(directory)):
        ...
        for i in range(len(ret_boxes)):
            cv2.imshow(
                'object detection', image_np[ret_boxes[i][1]:ret_boxes[i][3],
                                             ret_boxes[i][0]:ret_boxes[i][2]])
            img = cv2.resize(
                image_np[ret_boxes[i][1]:ret_boxes[i][3],
                         ret_boxes[i][0]:ret_boxes[i][2]], (224, 224))
